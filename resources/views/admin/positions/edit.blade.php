...
@section('content')
    <div class="animate-fade-in-up">
        <div class="flex items-center gap-3">
            ...
            <h1 class="font-display text-2xl font-bold text-slate-900">Update Lowongan</h1>
        </div>
    </div>
    ...
    <div class="mt-5 card p-6 max-w-2xl animate-fade-in-up delay-1">
        <form method="post" action="{{ route('admin.positions.update', $position) }}" class="space-y-5">
            ...
            <div>
                ...
                <input name="name" value="{{ old('name', $position->name) }}" required class="input-modern mt-1.5" />
            </div>
            <div>
                <label class="text-sm font-medium text-slate-700">Slug</label>
                <input name="slug" value="{{ old('slug', $position->slug) }}" class="input-modern mt-1.5" />
            </div>
            <div>
                <label class="text-sm font-medium text-slate-700">Deskripsi <span class="text-slate-400">(opsional)</span></label>
                <textarea name="description" rows="3" class="input-modern mt-1.5">{{ old('description', $position->description) }}</textarea>
            </div>
            <div>
                <label class="text-sm font-medium text-slate-700">Persyaratan melamar <span class="text-slate-400">(opsional)</span></label>
                <textarea name="requirements" rows="5" placeholder="Tulis per baris" class="input-modern mt-1.5">{{ old('requirements', $position->requirements) }}</textarea>
            ...
            <div class="grid gap-4 md:grid-cols-2">
                <div>
                    ...
                    <input type="number" name="sort_order" value="{{ old('sort_order', $position->sort_order) }}" class="input-modern mt-1.5" />
                </div>
                <div class="flex items-center gap-2 pt-7">
                    <input type="checkbox" name="is_active" value="1" class="rounded border-slate-300 text-indigo-600 focus:ring-indigo-500" @checked(old('is_active', $position->is_active)) />
                    <span class="text-sm font-medium text-slate-700">Aktif</span>
                </div>
            </div>
            <div class="flex gap-2 pt-2 border-t border-slate-100">
                <a href="{{ route('admin.positions.index') }}" class="btn-secondary">Kembali</a>
